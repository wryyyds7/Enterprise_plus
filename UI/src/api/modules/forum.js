...
export default {
  ...
  forumSection: {
    // 获取论坛板块列表
    getSectionList: (params) => request.get('/api/forum/section', { params }),
    // 获取论坛板块详情
    getSectionInfo: (sectionId) => request.get(`/api/forum/section/${sectionId}`),
    // 创建论坛板块
    createSection: (data) => request.post('/api/forum/section', data),
    // 更新论坛板块
    updateSection: (data) => request.put('/api/forum/section', data),
    // 删除论坛板块
    deleteSection: (sectionId) => request.delete(`/api/forum/section/${sectionId}`),
    // 批量删除论坛板块
    batchDeleteSection: (data) => request.delete('/api/forum/section/batch', data),
    // 查询子板块列表
  ...
  forumTopic: {
    // 获取论坛主题列表
    getTopicList: (params) => request.get('/api/forum/topic', { params }),
    // 获取论坛主题详情
    getTopicInfo: (topicId) => request.get(`/api/forum/topic/${topicId}`),
    // 创建论坛主题
    createTopic: (data) => request.post('/api/forum/topic', data),
    // 更新论坛主题
    updateTopic: (data) => request.put('/api/forum/topic', data),
    // 删除论坛主题
    deleteTopic: (topicId) => request.delete(`/api/forum/topic/${topicId}`),
    // 批量删除论坛主题
    batchDeleteTopic: (data) => request.delete('/api/forum/topic/batch', data),
    // 根据板块ID查询主题列表
  ...
  forumReply: {
    // 获取论坛回复列表
    getReplyList: (params) => request.get('/api/forum/reply', { params }),
    // 获取论坛回复详情
    getReplyInfo: (replyId) => request.get(`/api/forum/reply/${replyId}`),
    // 创建论坛回复
    createReply: (data) => request.post('/api/forum/reply', data),
    // 更新论坛回复
    updateReply: (data) => request.put('/api/forum/reply', data),
    // 删除论坛回复
    deleteReply: (replyId) => request.delete(`/api/forum/reply/${replyId}`),
    // 批量删除论坛回复
    batchDeleteReply: (data) => request.delete('/api/forum/reply/batch', data),
    // 根据主题ID查询回复列表
  ...
  forumFavorite: {
    // 获取论坛收藏记录列表
    getFavoriteList: (params) => request.get('/api/forum/favorite', { params }),
    // 获取论坛收藏记录详情
    getFavoriteInfo: (favoriteId) => request.get(`/api/forum/favorite/${favoriteId}`),
    // 创建论坛收藏记录
    createFavorite: (data) => request.post('/api/forum/favorite', data),
    // 删除论坛收藏记录
    deleteFavorite: (favoriteId) => request.delete(`/api/forum/favorite/${favoriteId}`),
    // 批量删除论坛收藏记录
    batchDeleteFavorite: (data) => request.delete('/api/forum/favorite/batch', data),
    // 根据主题ID和用户ID查询收藏记录
    ...
    // 收藏主题
    favoriteTopic: (data) => request.post('/api/forum/favorite/topic', data),
    // 取消收藏主题
